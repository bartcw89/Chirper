        ...
        <div class="flex-1">
            <div class="flex justify-between items-center">
                ...
                <Dropdown v-if="chirp.user.id === $page.props.auth.user.id">
                    ...
                    <template #content>
                        <button class="block px-4 py-2 w-full text-sm leading-5 text-left text-gray-700 transition duration-150 ease-in-out hover:bg-gray-100 focus:bg-gray-100" @click="editing = true">
                            Edit
                        </button>
            ...
            <form v-if="editing" @submit.prevent="form.put(route('chirps.update', chirp.id), { onSuccess: () => editing = false })">
                <textarea v-model="form.message" class="mt-4 w-full text-gray-900 rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"></textarea>
